...
class IntentRec(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # save the features
        # feats = outputs.cpu().detach().numpy()
        # np.save(os.path.join("../data/wav2vec2_feats/train", batch.id[0]), feats)

        outputs_req = self.modules.out_request(outputs)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = self.hparams.compute_cost(outputs_req, req_lab.squeeze(-1))
    ...
    def run_inference(
        ...
        with torch.no_grad():
            ...
            for batch in dataset:
                ...
                # USED FOR ONE TASK
                #true_labels_req.append(req_lab)
                #pred_labels_req.append(topk_req)

                # USED FOR MULTITASK (4 CLASSES)
        ...
        print("UAR Requests: %.4f" % balanced_accuracy_score(true_req, pred_req))
        print("UAR Complaints: %.4f" % balanced_accuracy_score(true_com, pred_com))
...
def data_prep(data_folder, hparams):
    ...
    def audio_pipeline(file_path):
        ...
        #sig = sig[:hparams["audio_length"]*16000]
